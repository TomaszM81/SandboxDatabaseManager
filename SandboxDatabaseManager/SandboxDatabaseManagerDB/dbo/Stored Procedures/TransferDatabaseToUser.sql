CREATE PROCEDURE dbo.TransferDatabaseToUser
(
	 @DatabaseName NVARCHAR(128),
	 @DatabaseOwner NVARCHAR(100),
	 @NewDatabaseOwner NVARCHAR(100)
)
...
BEGIN TRY
	...
	IF @DatabaseOwner = @NewDatabaseOwner
		RETURN 0
	...
	/*************************************    Removing Permission from old Owner   ********************************************************/

	EXEC [dbo].[RemovePermissionsFromUserOnDatabase] @DatabaseName, @DatabaseOwner

	/*************************************    Transfers Database Ownership    ********************************************************/

	INSERT INTO dbo.DatabaseOwnerTransfers (DatabaseName, OldDatabaseOwner)
	SELECT @DatabaseName, @DatabaseOwner WHERE @DatabaseOwner <> @NewDatabaseOwner;
	
	UPDATE dbo.Databases
    SET  DatabaseOwner = @NewDatabaseOwner
    WHERE DatabaseName = @DatabaseName and DatabaseOwner = @DatabaseOwner;
	
	
	/*************************************    Adding New User   ********************************************************/

	EXEC [dbo].[AddUserToDatabase] @DatabaseName, @NewDatabaseOwner
