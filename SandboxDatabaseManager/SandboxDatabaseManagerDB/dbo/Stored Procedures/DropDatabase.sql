...
BEGIN TRY
	...
	BEGIN
		RAISERROR('You are not allowed to drop someone else''s database.',11,0);
	END
	...
	DECLARE @SnapshotToRemove NVARCHAR(128)

	DECLARE curRemoveSnapshots CURSOR FAST_FORWARD FOR
	SELECT name FROM sys.databases WHERE source_database_id = DB_ID(@DatabaseName);

	OPEN curRemoveSnapshots

	FETCH NEXT FROM curRemoveSnapshots INTO @SnapshotToRemove
	
	WHILE @@FETCH_STATUS = 0
	BEGIN
		RAISERROR('Droping database snapshot [%s].', 0, 0, @SnapshotToRemove) WITH NOWAIT;	
		EXEC dbo.KillConnections @SnapshotToRemove;

		SET @Statement = 'DROP DATABASE ' + QUOTENAME(@SnapshotToRemove);
		EXEC(@Statement);

		FETCH NEXT FROM curRemoveSnapshots INTO @SnapshotToRemove
	END

	CLOSE curRemoveSnapshots
	DEALLOCATE curRemoveSnapshots
	...
	RAISERROR('Database [%s] removed.', 10, 1, @DatabaseName) WITH NOWAIT;	
