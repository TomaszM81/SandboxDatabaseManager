CREATE PROCEDURE [dbo].[ChangeTrackingDisable]
(
...
SET NOCOUNT ON
	...
	INSERT @Tables
		SELECT
			...
			''ALTER TABLE '' + QUOTENAME(OBJECT_SCHEMA_NAME(st.object_id)) + ''.'' + QUOTENAME(st.Name) + '' DISABLE CHANGE_TRACKING'',
	...
	BEGIN
		 EXEC(@SqlStatement);
		 RAISERROR(''Disabled change tracking for %s table, %d of %d tables done.'', 10, 1, @Name, @currentID, @totalCount ) WITH NOWAIT;
	...
	IF EXISTS(select * from sys.change_tracking_databases ctd INNER JOIN sys.databases sd ON ctd.database_id = sd.database_id WHERE sd.name = @DatabaseName)
	BEGIN
		SET @SqlStatement = 'ALTER DATABASE ' + QUOTENAME(@DatabaseName) + ' SET CHANGE_TRACKING = OFF';
		EXEC(@SqlStatement);
	END
